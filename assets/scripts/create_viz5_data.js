...
d3.queue()
    ...
    .defer(d3.json, "assets/data/final_network.json")
    .defer(d3.json, "assets/data/viz_5_distinctRoutes.json")
    .await(buildVizFive);
...
function buildVizFive(error, stationData, networkData, yearData) {
    ...
    distinctRoutes = distinctRoutesData;
    ...
    for (let i = 0; i < yearData.length; i++) {
        ...
        if (start != end) {
            if (!objectInArray({start: start, end: end}, distinctRoutes)) {
                distinctRoutes.push({start: start, end: end});
            }
    ...
    console.log("Making API calls to OSRM for Route Data...");
    // let no = ['Test 000e3', '8D QC Station 02', '8D QC Station 01'];
    let numRoutes = 0;
    for (let i = 0, p = Promise.resolve(); i < 3000; i++) {
        console.log(i, distinctRoutes.length);

        p = p.then(_ => {
            let start = distinctRoutes[i]['start'];
            let end = distinctRoutes[i]['end'];
            if (!no.includes(start) && !no.includes(end)) {
                console.log("hi");
                new Promise(resolve => {
                    ...
                    route.then((result) => {
                        ...
                        let distance = 0;
                        for (let j = 0; j < result.geometry.coordinates.length - 1; j++) {
                            let d = coordDistance(result.geometry.coordinates[j][0], result.geometry.coordinates[j][1], result.geometry.coordinates[j+1][0], result.geometry.coordinates[j+1][1]) * 1000;
                            distance += d;
                        }
                        stationNetwork[start][end]['geoJSON'] = result.geometry;
                        stationNetwork[start][end]['meters'] = distance;
                        console.log(numRoutes);
                        if (numRoutes == 3000 - 1) {
                            download(JSON.stringify(stationNetwork), 'final_network.json', 'application/json');
                        }
                        numRoutes+=1;
                        resolve();
                    });
